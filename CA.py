...
class Project(object):
    ...
    def generate_content(self):
        my_content = Content(self)
        my_content.create_all()

    def generate_quotation(self):
        my_quotation = Quotation(self)
        my_quotation.create_all()
...
class Quotation(object):
    ...
    def create_general(self):
        ...
        for i in range(colum_number):
            for j in range(1, row_number):
                ...
                if i == 1 and j > 1:
                    cell_now.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
                elif i == 2 and j > 1:
                    cell_now.alignment = Alignment(horizontal='right', vertical='center', wrap_text=True)
        ...
        self.ws_general['C3'] = "='3.物资对内分项报价表'!M{}".format(len(self.project.commodities) + 3)
        ...
        self.ws_general["B{}".format(real_row_num - 1)] = "其他费用{}含须中方承担的其他费用、管理费、风险预涨费、防恐措施费（如有）".format(linesep)
        self.ws_general['C{}'.format(real_row_num - 1)] = "=费用输入!J14"
        self.ws_general['B{}'.format(real_row_num)] = '合计'
        self.ws_general['C{}'.format(real_row_num)] = "=SUM(C3:C{})".format(real_row_num - 1)
    ...
    def create_input(self):
        ...
        for num in range(2, row_number + 1):
            if self.project.commodities[num - 1][-1] == '':
                ...
                self.ws_input['A{}'.format(num)] = self.project.commodities[num - 1][-1]  # 填写物资序号
            ...
            for rela in relate_coord:
                self.ws_input['{}{}'.format(rela[0], num)] = self.project.commodities[num - 1][rela[1]]
            else:
                self.ws_input['E{}'.format(num)].number_format = '0'
                self.ws_input['E{}'.format(num)] = int(self.project.commodities[num - 1][3])
    ...
    def create_cost(self):
        ...
        colum_title = ['海运', '单价', '', '数量', '总金额', '陆运', '单价', '', '数量', '总金额']
        ...
        for i in range(2, 19):  # D列格式及E列格式
            ...
            self.ws_cost['E{}'.format(i)].number_format = '¥#,##0.00'
            if i > 4:
                self.ws_cost['E{}'.format(i)] = '=C{0}*D{0}'.format(i)  #E 列公式生成
                self.ws_cost['D{}'.format(i)].number_format = '0'
    ...
    def create_selection(self):
        ...
        # 填入数据
        col_relate = [('A', 'A'), ('B', 'B'), ('D', 'G'), ('F', 'J'), ('G', 'V'), ('H', 'W')]
        for row in range(3, row_number+1):  # 遍历行
            ...
            self.ws_selection['C{}'.format(row)] = self.project.commodities[row-2][4]  # 填入招标要求
    ...
    def create_itemized_quotation(self):
        ...
        for i in range(colum_number):
            for j in range(1, row_number):  # 留出第一行
                cell_now = self.ws_itemized_quotation.cell(row=j + 1, column=i + 1)
                if j < 2:
                    cell_now.font = bold_font
                    cell_now.alignment = ctr_alignment
                else:
        ...
        self.ws_itemized_quotation['B{}'.format(row_number)] = '3.第10项应纳增值税=[对内总承包价/（1+增值税税率）]X增值税税率-当期进项税款'
        self.ws_itemized_quotation['B{}'.format(row_number - 2)].fill = yellow_fill
        ...
        for row in range(3, row_number - 3):
            for col in col_relate:  # 根据对应关系设立公式
                self.ws_itemized_quotation['{}{}'.format(col[0], row)] = '=物资输入!{}{}'.format(col[1], row-1)
            ...
            self.ws_itemized_quotation['G{}'.format(row)] = '=C{0}/C{1}*G{1}'.format(row, row_number - 3)
            self.ws_itemized_quotation['H{}'.format(row)] = '=C{0}/C{1}*H{1}'.format(row, row_number - 3)
            self.ws_itemized_quotation['I{}'.format(row)] = '=C{0}/C{1}*I{1}'.format(row, row_number - 3)
            self.ws_itemized_quotation['J{}'.format(row)] = '=C{0}/C{1}*J{1}'.format(row, row_number - 3)
            self.ws_itemized_quotation['K{}'.format(row)] = '=C{0}/C{1}*K{1}'.format(row, row_number - 3)
            self.ws_itemized_quotation['L{}'.format(row)] = '=C{0}/C{1}*L{1}'.format(row, row_number - 3)
            self.ws_itemized_quotation['M{}'.format(row)] = '=SUM(C{0}:L{0})'.format(row)
        self.ws_itemized_quotation['C{}'.format(row_number - 3)] = '=SUM(C3:C{})'.format(row_number - 4)
        self.ws_itemized_quotation['D{}'.format(row_number - 3)] = '=SUM(D3:D{})'.format(row_number - 4)
        self.ws_itemized_quotation['E{}'.format(row_number - 3)] = '=SUM(E3:E{})'.format(row_number - 4)
        self.ws_itemized_quotation['F{}'.format(row_number - 3)] = '=SUM(F3:F{})'.format(row_number - 4)
        ...
        self.ws_itemized_quotation['K{}'.format(row_number - 3)] = '=IF(C{0}>50000000,(C{0}-50000000)*0.0075+835000,' \
                                                                   ...
                                                                   '*0.04)))))'.format(row_number - 3)
        self.ws_itemized_quotation['L{}'.format(row_number - 3)] = '=M{0}/1.13*0.13-C{0}/1.13*0.13-G{0}/1.06*0.06'.format(
            row_number - 3)
        self.ws_itemized_quotation['M{}'.format(row_number - 3)] = '=SUM(C{0}:L{0})'.format(row_number - 3)
        self.ws_itemized_quotation['J{}'.format(row_number - 3)] = '=SUM(C{0}:I{0})*3/12*0.0435'.format(row_number - 3)
        self.ws_itemized_quotation['J{}'.format(row_number - 3)].fill = yellow_fill

        # 合并需要合并单元格
        self.ws_itemized_quotation.merge_cells('A1:M1')
        self.ws_itemized_quotation.merge_cells('B{0}:M{0}'.format(row_number - 1))
        self.ws_itemized_quotation.merge_cells('B{0}:M{0}'.format(row_number - 2))
        self.ws_itemized_quotation.merge_cells('B{0}:M{0}'.format(row_number))

    def create_summed_quotation(self):
        """创建对内总报价表"""
        self.ws_summed_quotationn = self.wb.create_sheet('2.物资对内总报价表', 3)
        ...
        for i in range(colum_number):
            for j in range(1, row_number):
                cell_now = self.ws_summed_quotationn.cell(row=j + 1, column=i + 1)
                if j < row_number - 1:
                    cell_now.border = full_border
                if j < 2:
                    cell_now.font = bold_font
                ...
                cell_now.alignment = ctr_alignment
        for i in range(len(title_width)):  # 修改列宽
            self.ws_summed_quotationn.column_dimensions[
                self.ws_summed_quotationn.cell(row=4, column=i + 1).column_letter].width = title_width[i]
        for i in range(2, row_number):  # 修改行高
            self.ws_summed_quotationn.row_dimensions[
                self.ws_summed_quotationn.cell(row=i, column=1).row].height = 30
        self.ws_summed_quotationn.row_dimensions[row_number].height = 45

        # 创建标题行
        self.ws_summed_quotationn['A1'].font = title_font
        self.ws_summed_quotationn['A1'].alignment = ctr_alignment
        self.ws_summed_quotationn['A1'] = '2.物资对内总报价表'
        self.ws_summed_quotationn.row_dimensions[1].height = 30

        # 填写表头
        index = 0
        for i in self.ws_summed_quotationn['A2':'k2'][0]:
            # print(index+1, i)
            if colum_title[index] != '':
                i.value = colum_title[index]
                # i.font = bold_font
        ...
        for row in range(3, row_number - 1):  # 遍历行
            for col in col_relate:  # 根据对应关系设立公式
                cell_now = self.ws_summed_quotationn['{}{}'.format(col[0], row)]
                cell_now.value = '=物资输入!{}{}'.format(col[1], row - 1)
                if col[0] == 'C':
                    cell_now.alignment = left_alignment
            self.ws_summed_quotationn['J{}'.format(row)] = "='3.物资对内分项报价表'!M{}".format(row)
            self.ws_summed_quotationn['J{}'.format(row)].number_format = '#,##0.00'
            self.ws_summed_quotationn['G{}'.format(row)] = '=J{0}/I{0}'.format(row)
            self.ws_summed_quotationn['G{}'.format(row)].number_format = '#,##0.00'
        self.ws_summed_quotationn['A{}'.format(row_number - 1)] = '合计金额'
        self.ws_summed_quotationn['C{}'.format(row_number - 1)] = '=SUM(J3:J{})'.format(row_number - 1)
        self.ws_summed_quotationn['C{}'.format(row_number - 1)].font = bold_font
        self.ws_summed_quotationn['C{}'.format(row_number - 1)].number_format = '¥#,##0.00'
        self.ws_summed_quotationn['C{}'.format(row_number - 1)].alignment = Alignment(
            horizontal='left', vertical='center', wrap_text=True)
        self.ws_summed_quotationn['A{}'.format(row_number)] = '注：'
        self.ws_summed_quotationn['B{}'.format(row_number)] = "（1）本表所列{}{}单价和总价包括投标人提供上述全部物资" \
                                                              "并承担合同规定全部义务所需的一切费用；{}（2）在备注栏中注明包装" \
                                                              "情况,即包装的单位和数量".format(self.project.trans,
                                                                                    self.project.destination, linesep)
        self.ws_summed_quotationn['B{}'.format(row_number)].alignment = Alignment(horizontal='left',
                                                                                      vertical='center', wrap_text=True)

        # 合并需要合并单元格
        self.ws_summed_quotationn.merge_cells('A1:K1')
        self.ws_summed_quotationn.merge_cells('H2:I2')
        self.ws_summed_quotationn.merge_cells('A{0}:B{0}'.format(row_number - 1))
        self.ws_summed_quotationn.merge_cells('C{0}:K{0}'.format(row_number - 1))
        self.ws_summed_quotationn.merge_cells('B{0}:K{0}'.format(row_number))
    ...
    def create_examination(self):
        ...
        title_width = [6, 14, 12, 3, 5, 10, 30, 16, 16, 16, 10, 10, 10, 10, 8, 18, 13, 8, 7, 6]
        ...
        for col in range(1, 21):
            if col not in [4, 5, 12, 13, 17, 18, 19]:
                self.ws_examination.merge_cells(start_row=2, start_column=col, end_row=3, end_column=col)
        for i in range(len(title_width)):  # 修改列宽
        ...
        col_relate = [('A', 'A'), ('B', 'B'), ('C', 'C'), ('D', 'D'), ('E', 'E'), ('F', 'F'), ('G', 'G'), ('H', 'I'),
                      ('I', 'J'), ('J', 'K'), ('K', 'M'), ('L', 'N'), ('M', 'O'), ('N', 'P'), ('O', 'Q'),  ('P', 'R'),
                      ('Q', 'S'), ('R', 'T'), ('S', 'U'), ('T', 'Y')]
    ...
    def create_techserve(self):
        ...
        colum_title = ['序号', '费用名称', '外币单价', '人民币单价', '人数', '天/次数', '外币合计', '人民币合计']
        ...
        # 填写数据
        col_a = [1, 2, 3, 4, 5, 6, 7, '7-1', '7-2', '7-3', '7-4', 8, 9, '']  # 序号
        ...
        for num in range(3):  # 填写技术服务费单价
            self.ws_techserve['C{}'.format(num+6)].number_format = '$#,##0.00'
            self.ws_techserve['C{}'.format(num+6)] = self.project.techinfo[2][num]
        # 格式化单元格
        for i in [3, 4, 5, 6, 7, 8, 14, 16]:
            ...
            if i < 9:
                ...
                if i in [3, 6, 7, 8]:
                    self.ws_techserve['F{}'.format(i)] = self.project.techinfo[1]
            if i < 6:
    ...
    def create_lawexam(self):
        ...
        colum_title = ['序号', '品名', 'HS编码', '数量及单位', '', '品牌', '规格、技术参数或配置', '金额', '生产厂商',
                       ...
                       '口岸检验机构', '备注']
        title_width = [6, 14, 12, 3, 5, 10, 30, 16, 16, 16, 10, 10, 10, 10, 8, 18, 7, 6]
        ...
        for row in range(3, row_number + 1):  # 遍历行
            for col in col_relate:  # 根据对应关系设立公式
                ...
                cell_now.value = '=物资输入!{}{}'.format(col[1], self.project.qc[index] + 1)
                if col[0] == 'G':
                    cell_now.alignment = left_alignment
                if col[0] == 'H':
                    cell_now.number_format = '¥#,##0.00'
...
my_project.generate_content()
my_project.generate_quotation()
